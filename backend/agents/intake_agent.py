...
async def _classify_contract(parsed_doc: ParsedDocument) -> ContractMetadata:
    """
    Use Gemini to classify the contract and extract metadata.
    """
    ...
    try:
        response_text = await llm_generate(
            ...
            user_prompt=f"Analyze this contract and extract metadata:\n\n{sample_text}",
            max_tokens=1024,
        )

        classified = parse_json_response(response_text)
        if isinstance(classified, list):
            classified = classified[0] if classified else {}
        ...
        logger.warning(f"⚠️  LLM classification failed: {e}. Using defaults.")
